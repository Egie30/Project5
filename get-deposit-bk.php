...
$query_rev	= "SELECT 
	HED.ORD_NBR,
	SUM(CASE WHEN HED.ACTG_TYP = 1 THEN HED.TND_AMT ELSE 0 END) AS REV_PT,
	SUM(CASE WHEN HED.ACTG_TYP = 2 THEN HED.TND_AMT ELSE 0 END) AS REV_CV,
	SUM(CASE WHEN HED.ACTG_TYP = 3 THEN HED.TND_AMT ELSE 0 END) AS REV_PR
FROM 
(
SELECT 
										CSH.TRSC_NBR,
									...
									FROM 
											(	SELECT
													...
													WHERE ACT_F = 0
														AND CSH_FLO_TYP = 'FL'
														AND DATE(CRT_TS) >= (SELECT BEG_DEP FROM NST.PARAM_LOC)
												...
												LEFT JOIN (
													...
													WHERE REG.ACT_F = 0
														...
														AND DATE(REG.CRT_TS) >= (SELECT BEG_DEP FROM NST.PARAM_LOC)
										...
										WHERE HED.PAID_DT IS NOT NULL 
											AND DATE(HED.ORD_TS) >= (SELECT BEG_DEP FROM NST.PARAM_LOC)
											AND DATE(HED.PAID_DT) < CURRENT_DATE
										GROUP BY CSH.TRSC_NBR, CSH.RTL_BRC
										ORDER BY PYMT.PYMT_TYP_ORD ASC
) HED 
";

$result_rev	= mysql_query($query_rev);
$row_rev	= mysql_fetch_array($result_rev);

$RevenuePT	= $row_rev['REV_PT'];
$RevenueCV	= $row_rev['REV_CV'];
$RevenuePR	= $row_rev['REV_PR'];
*/
				...
				( SELECT 
						...
						LEFT JOIN (
							...
							WHERE PYMT.DEL_NBR = 0
								AND HED.DEL_NBR = 0
								AND DATE(PYMT.CRT_TS) >= (SELECT BEG_DEP FROM NST.PARAM_LOC)
								AND DATE(HED.ORD_TS) >= (SELECT BEG_DEP FROM NST.PARAM_LOC)
...
$query_exp	= "SELECT 
				...
				WHERE DATE(CSH.CRT_TS) >= (SELECT BEG_DEP FROM NST.PARAM_LOC)
					AND DATE(CSH.CRT_TS) <= CURRENT_DATE
				...
				FROM (
					...
					FROM 
							( SELECT 
								...
								WHERE DATE(CSH.CRT_TS) >= (SELECT BEG_DEP FROM NST.PARAM_LOC)
									AND DATE(CSH.CRT_TS) <= CURRENT_DATE
									AND DATE(HED.ORD_DTE) >= (SELECT BEG_DEP FROM NST.PARAM_LOC)
...
/*
$InvoiceAmountPT	= 0;
$InvoiceAmountCV	= $row_ivc['TND_AMT'];
$InvoiceAmountPR	= 0;
*/
...
$query_dep	= "SELECT 
				...
				WHERE CSH.CSH_DAY_DTE <= CURRENT_DATE
					AND CSH.DEL_NBR = 0";
...
$satuan_csh 	= 1000000;
	...
	if ((($RevenueCV + $OmsetTodayCV + $RevenueRetail) - $depCV - $ExpenseAmount - $InvoiceAmountCV) >= $MinCashCV)	{ $DepositCV	= (($RevenueCV + $OmsetTodayCV + $RevenueRetail) - $depCV - $ExpenseAmount - $InvoiceAmountCV) - ($MinCashCV); }	
		else { $DepositCV = 0; }
