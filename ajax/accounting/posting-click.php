...
foreach($results->data as $resultTransaction) {
	...
	if(($resultTransaction->TOT_AMT) != 0) {
		...
		//echo $query."<br />";
			...
			$query	= "UPDATE RTL.ACCTG_GL_DET SET DEB =".$resultTransaction->TOT_AMT." WHERE GL_NBR=".$glNumber." AND CD_SUB_NBR=".$CdSubNbr." ";
			mysql_query($query);
			
			//echo $query."<br />";
			
			$query	= "UPDATE RTL.ACCTG_GL_DET SET CRT =".$resultTransaction->TOT_AMT." WHERE GL_NBR=".$glNumber." AND CD_SUB_NBR= ".$CdSubBalance." ";
			mysql_query($query);
