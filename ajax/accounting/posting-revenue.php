...
foreach($ArrayActg as $Actg) {
	...
	try {
		ob_start();
		include __DIR__ . DIRECTORY_SEPARATOR . "../prn-dig-report-tax.php";
...
$query_ppn	= "SELECT CD_SUB_NBR FROM RTL.ACCTG_GL_TYP WHERE GL_TYP_NBR = 21"; //PPN Keluaran
$result_ppn	= mysql_query($query_ppn);
$row_ppn	= mysql_fetch_array($result_ppn);

$CdSubPPN	= $row_ppn['CD_SUB_NBR'];

$CdSubBalance	= 6; 		//Aktiva Piutang Dagang

	foreach($resultsPrint->data as $resultTransaction) {
	
	if(($resultTransaction->TOT_AMT) != 0) {
				
		$query		= "SELECT GL_NBR FROM RTL.ACCTG_GL_HEAD WHERE REF='".$GLType."/".$resultTransaction->CSH_DTE."' AND GL_TYP_NBR=".$row_typ['GL_TYP_NBR']." AND ACTG_TYP = ".$Actg." ";
		...
		$query       = "INSERT INTO RTL.ACCTG_GL_HEAD(GL_NBR, BK_NBR, GL_DTE,  GL_DESC, REF, CRT_TS, CRT_NBR, GL_TYP_NBR, ACTG_TYP)
			VALUES (" . $glNumber . ", '" . $bookNumber . "', '" . $resultTransaction->CSH_DTE . "', 'Jurnal ".$GLDesc." (System)', '".$GLType."/" . $resultTransaction->CSH_DTE . "', CURRENT_TIMESTAMP, " . $_GET['personNBR'] . ", ".$GLTypeNbr.", ".$Actg.")";
		mysql_query($query);
		...
		$glDetailNumber++;
		$query     = "INSERT INTO RTL.ACCTG_GL_DET (GL_DET_NBR, GL_NBR, CD_SUB_NBR, DEB, CRT, UPD_NBR)
			VALUES (" . $glDetailNumber . ", " . $glNumber . ", ".$CdSubNbr.", 0, " . $resultTransaction->SUBTOTAL . ", " . $_GET['personNBR'] . ")";
		mysql_query($query);
		
		//echo $query."<br />";
	
			if($resultTransaction->TAX_APL_ID != 'E') {
				
				$glDetailNumber++;
				$query     = "INSERT INTO RTL.ACCTG_GL_DET (GL_DET_NBR, GL_NBR, CD_SUB_NBR, DEB, CRT, UPD_NBR)
					VALUES (" . $glDetailNumber . ", " . $glNumber . ", ".$CdSubPPN.", 0, " . $resultTransaction->TAX_AMT . ", " . $_GET['personNBR'] . ")";
				mysql_query($query);
...
include __DIR__ . DIRECTORY_SEPARATOR . "posting-revenue-payment.php";
