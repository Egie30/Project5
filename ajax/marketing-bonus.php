<?php
require_once __DIR__ . "/../framework/database/connect-cloud.php";
...
$resultGlbl	= mysql_query($queryGlbl, $local);
...
$resultSpv	= mysql_query($querySpv, $local);
...
$query = "SELECT
	...
	SUM(CASE WHEN ACT_F = 0 AND BNS_AMT > 0 THEN COALESCE(TND_AMT,0)-(COALESCE(TND_AMT,0)*(DEPN_PCT/100)) ELSE 0 END) AS TND_AMT_NETT_APV,
	SUM(CASE WHEN ACT_F = 0 AND BNS_AMT > 0 THEN COALESCE(TND_AMT,0) ELSE 0 END) AS TND_AMT_APV,
	SUM(CASE WHEN ACT_F = 1 AND BNS_AMT > 0 THEN COALESCE(TND_AMT,0) ELSE 0 END) AS TND_AMT_NAPV,
	BNS.BNK_CO_NBR,
	VAL_NBR,
	DEPN_PCT,
	SUM(COALESCE(BNS_AMT,0)) AS BNS_AMT,
	FLOOR(SUM(
		CASE WHEN ACT_F = 0 AND BNS_AMT > 0 THEN (COALESCE(TND_AMT,0)-(COALESCE(TND_AMT,0)*(DEPN_PCT/100)))/".$minPayment."
		ELSE 0 
	END))* ".$bonusMkt." AS TOT_BNS,
	(FLOOR(SUM(
		CASE WHEN ACT_F = 0 AND BNS_AMT > 0 THEN (COALESCE(TND_AMT,0)-(COALESCE(TND_AMT,0)*(DEPN_PCT/100)))/".$minPayment."
		ELSE 0 
	END))* ".$bonusMkt.") * ".$bonusSpv." AS TOT_BNS_SPV,
...
INNER JOIN $CMP.PEOPLE PPL ON BNS.ACCT_EXEC_NBR = PPL.PRSN_NBR AND PPL.TERM_DTE IS NULL AND PPL.DEL_NBR = 0
WHERE ".$whereClauses." 
