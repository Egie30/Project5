...
$whereClauses = array("DATE(HED.ORD_DTE) >= (SELECT BEG_ACCTG FROM NST.PARAM_LOC)","(HED.BUY_CO_NBR NOT IN (SELECT CO_NBR FROM NST.PARAM_COMPANY) OR HED.BUY_CO_NBR IS NULL)");
$groupClauses = array();

$whereClauses[] = "HED.SEL_CO_NBR =" . $companyNumber;
...
$query = "SELECT 
	...
	(CASE 
		WHEN HED.BUY_CO_NBR != '' THEN RCV.NAME 
		ELSE 'Tunai' 
	END) AS BUY_NAME
FROM CMP.CAL_ORD_HEAD HED
	LEFT OUTER JOIN CMP.COMPANY SHP ON SHP.CO_NBR = HED.SEL_CO_NBR
	LEFT OUTER JOIN CMP.COMPANY RCV ON RCV.CO_NBR = HED.BUY_CO_NBR
WHERE " . $whereClauses . "
