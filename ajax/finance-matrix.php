...
*/
//RUMUS OMSET LAMA(TANPA DEPRESIASI)
...
if($beginDate==date('Y-m-01')){
	$queryIN = "SELECT SUM(TOT_SUB) AS TOT_OMSET FROM (
		SELECT 
			SUM(CASE
				WHEN
					DATE(PAY.CRT_TS) <= (DATE_ADD(DATE(BLJRN.BILL_DTE), INTERVAL COALESCE(COM.PAY_TERM,0) + 30 DAY))
					THEN PAY.TND_AMT - ((PAY.TND_AMT  * 1)/100) * (100 / 100)
				WHEN
					DATE(PAY.CRT_TS) > (DATE_ADD(DATE(BLJRN.BILL_DTE), INTERVAL COALESCE(COM.PAY_TERM,0) + 30 DAY))
					AND DATE(PAY.CRT_TS) <= (DATE_ADD(DATE_ADD(DATE(BLJRN.BILL_DTE), INTERVAL COALESCE(COM.PAY_TERM,0) + 30 DAY),INTERVAL 30 DAY))
					THEN PAY.TND_AMT - (((PAY.TND_AMT  * 1)/100) - ((PAY.TND_AMT  * 1)/100) * (20 / 100))
				WHEN
					DATE(PAY.CRT_TS) >  (DATE_ADD(DATE(BLJRN.BILL_DTE), INTERVAL COALESCE(COM.PAY_TERM,0) + 60 DAY))
					AND DATE(PAY.CRT_TS) <= (DATE_ADD(DATE_ADD(DATE(BLJRN.BILL_DTE), INTERVAL COALESCE(COM.PAY_TERM,0) + 60 DAY),INTERVAL 30 DAY))
					THEN PAY.TND_AMT - (((PAY.TND_AMT  * 1)/100) - ((PAY.TND_AMT  * 1)/100) * (40 / 100))
				WHEN
					DATE(PAY.CRT_TS) > (DATE_ADD(DATE(BLJRN.BILL_DTE), INTERVAL COALESCE(COM.PAY_TERM,0) + 90 DAY))
					AND DATE(BLJRN.BILL_DTE) <= (DATE_ADD(DATE_ADD(DATE(BLJRN.BILL_DTE), INTERVAL COALESCE(COM.PAY_TERM,0) + 90 DAY),INTERVAL 30 DAY))
					THEN PAY.TND_AMT - (((PAY.TND_AMT  * 1)/100) - ((PAY.TND_AMT  * 1)/100) * (60 / 100))
				WHEN
					DATE(PAY.CRT_TS) > (DATE_ADD(DATE(BLJRN.BILL_DTE), INTERVAL COALESCE(COM.PAY_TERM,0) + 120 DAY))
					AND DATE(PAY.CRT_TS) <= (DATE_ADD(DATE_ADD(DATE(BLJRN.BILL_DTE), INTERVAL COALESCE(COM.PAY_TERM,0) + 120 DAY),INTERVAL 30 DAY))
					THEN PAY.TND_AMT - (((PAY.TND_AMT  * 1)/100) - ((PAY.TND_AMT  * 1)/100) * (80 / 100))
				WHEN
					DATE(PAY.CRT_TS) > (DATE_ADD(DATE(BLJRN.BILL_DTE), INTERVAL COALESCE(COM.PAY_TERM,0) + 120 DAY))
					THEN PAY.TND_AMT - (((PAY.TND_AMT  * 1)/100) - ((PAY.TND_AMT  * 1)/100) * (0 / 100))
			END) AS TOT_SUB
			FROM CMP.PRN_DIG_ORD_PYMT PAY 
			LEFT JOIN CMP.PRN_DIG_ORD_HEAD HED ON PAY.ORD_NBR=HED.ORD_NBR AND HED.DEL_NBR=0
			...
			LEFT OUTER JOIN(
			   ...
			   WHERE ORD_STT_ID = ('BL')
			   GROUP BY ORD_NBR
			)BLJRN ON PAY.ORD_NBR = BLJRN.ORD_NBR
			LEFT OUTER JOIN(
				SELECT
					 ORD_NBR,
			   ...
			   WHERE ORD_STT_ID = ('CP')
			   GROUP BY ORD_NBR
			)CPJRN ON PAY.ORD_NBR = CPJRN.ORD_NBR
			WHERE PAY.DEL_NBR = 0 
			AND DATE(CPJRN.CP_DTE) >= '" . $beginDate . "' AND DATE(CPJRN.CP_DTE) <= '" . $endDate . "'
			AND (HED.BUY_CO_NBR NOT IN (SELECT CO_NBR FROM NST.PARAM_COMPANY WHERE ACTG_TYP IS NULL) OR HED.BUY_CO_NBR IS NULL)
		) T";
} else {
	$queryIN = "SELECT SUM(TOT_SUB) AS TOT_OMSET FROM (
