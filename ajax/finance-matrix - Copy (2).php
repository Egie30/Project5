...
if($beginDate==date('Y-m-01')){
	$queryIN = "SELECT SUM(TOT_SUB) AS TOT_OMSET FROM (
			...
			FROM CMP.PRN_DIG_ORD_PYMT_ARC_ALL PAY 
				LEFT JOIN CMP.PRN_DIG_ORD_HEAD_ARC_ALL HED ON PAY.ORD_NBR=HED.ORD_NBR AND HED.DEL_NBR=0
			WHERE PAY.DEL_NBR = 0 AND DATE(PAY.CRT_TS) BETWEEN '".$beginDate."' AND '".$endDate."'
			UNION ALL 
			SELECT COALESCE(SUM(PAY.TND_AMT), 0) AS TOT_SUB
			FROM CMP.PRN_DIG_ORD_PYMT PAY 
				LEFT JOIN CMP.PRN_DIG_ORD_HEAD HED ON PAY.ORD_NBR=HED.ORD_NBR
				AND HED.DEL_NBR=0
			WHERE PAY.DEL_NBR = 0 AND DATE(ORD_TS) = CURRENT_DATE
		) T";
} else {
	$queryIN = "SELECT SUM(TOT_SUB) AS TOT_OMSET FROM (
			SELECT COALESCE(SUM(PAY.TND_AMT), 0) AS TOT_SUB
