...
$whereClauses 	= array("HED.DEL_F=0", "(HED.DL_TS IS NOT NULL OR HEADER.DL_TS IS NOT NULL)","TYP.CAT_TYP_NBR IN (1,2)");
$whereMovement 	= array("HED.DEL_F=0", "(HED.DL_TS IS NOT NULL OR HEADER.DL_TS IS NOT NULL)", "MOV.DEL_NBR = 0", "TYP.CAT_TYP_NBR IN (1,2)");
...
//$whereClauses[]	= "HED.TOT_REM = 0";
	...
	$whereClauses[]		= "(DATE(HEADER.ORD_DTE) >= (SELECT BEG_ACCTG FROM NST.PARAM_LOC) OR DATE(HEADER.ORD_DTE) IS NULL)";
	$whereMovement[]	= "DATE(HED.ORD_DTE) >= (SELECT BEG_ACCTG FROM NST.PARAM_LOC)";
...
$query = "SELECT 
	...
	SUM(COALESCE(STK.RCV_TOT_SUB,0) + COALESCE(STK.XF_IN_TOT_SUB,0) - COALESCE(STK.RTR_TOT_SUB,0) + COALESCE(STK.COR_TOT_SUB,0) - COALESCE(STK.XF_OUT_TOT_SUB,0) - COALESCE(MOV.MOV_TOT_SUB,0)) AS BALANCE_AMT_BRUTO,
	COALESCE(SUM(
					CASE WHEN (STK.TAX_APL_ID = 'I' AND STK.ACTG_TYP = 1)
							THEN ((COALESCE(STK.RCV_TOT_SUB,0) + COALESCE(STK.XF_IN_TOT_SUB,0) - COALESCE(STK.RTR_TOT_SUB,0) + COALESCE(STK.COR_TOT_SUB,0) - COALESCE(STK.XF_OUT_TOT_SUB,0) - COALESCE(MOV.MOV_TOT_SUB,0)))/1.1 
						ELSE ((COALESCE(STK.RCV_TOT_SUB,0) + COALESCE(STK.XF_IN_TOT_SUB,0) - COALESCE(STK.RTR_TOT_SUB,0) + COALESCE(STK.COR_TOT_SUB,0) - COALESCE(STK.XF_OUT_TOT_SUB,0) - COALESCE(MOV.MOV_TOT_SUB,0)))
						END
					),0) AS BALANCE_AMT
FROM (SELECT
