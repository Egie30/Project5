...
if (empty($beginDate) && empty($endDate)) {
	if ($days != "") {
		$whereClauses[] = "DAY(HED.CMP_TS)=".$days;
	}

	if ($months != "") {
		$whereClauses[] = "MONTH(HED.CMP_TS)= ".$months;
	}

	if ($years != "") {
		$whereClauses[] = "YEAR(HED.CMP_TS)= ". $years;
	}

	if ($day != "") {
		$whereClauses[] = "DAY(HED.CMP_TS)=" . $day;
	}

	if ($month != "") {
		$whereClauses[] = "MONTH(HED.CMP_TS)=" . $month;
	}

	if ($year != "") {
		$whereClauses[] = "YEAR(HED.CMP_TS)=" . $year;
	}
} else {
	if (!empty($beginDate)) {
		$whereClauses[] = "DATE(HED.CMP_TS) >= '" . $beginDate . "'";
	}

	if (!empty($endDate)) {
		$whereClauses[] = "DATE(HED.CMP_TS) <= '" . $endDate . "'";
	}
}
...
if (count($groups) > 0) {
	...
	while(count($groups) > 0) {
		...
		switch ($group) {
			case "YEAR":
				$groupClauses[] = "YEAR(HED.CMP_TS)";
				break;
			case "MONTH":
				$groupClauses[] = "YEAR(HED.CMP_TS), MONTH(HED.CMP_TS)";
				break;
			case "DAY":
				$groupClauses[] = "YEAR(HED.CMP_TS), MONTH(HED.CMP_TS), DAY(HED.CMP_TS)";
				break;
			default:
...
if ($_GET['RL_TYP'] == 'RL_YEAR') {
	...
	$query = "SELECT MO.*, 
			...
			(
				...
				MO
				LEFT JOIN 
					(SELECT DATE(HED.CMP_TS) AS ORD_DTE,
							DATE(HED.CMP_TS) AS CSH_DTE,
							YEAR(HED.CMP_TS) AS CSH_YEAR,
							MONTH(HED.CMP_TS) AS CSH_MONTH,
							DAY(HED.CMP_TS) AS CSH_DAY,
							MONTHNAME(HED.CMP_TS) AS CSH_MONTHNAME,
							HED.ORD_NBR,
...
$query = "SELECT DATE(HED.CMP_TS) AS ORD_DTE,
			DATE(HED.CMP_TS) AS CSH_DTE,
			YEAR(HED.CMP_TS) AS CSH_YEAR,
			MONTH(HED.CMP_TS) AS CSH_MONTH,
			DAY(HED.CMP_TS) AS CSH_DAY,
