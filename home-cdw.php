...
#$CoEx	= "SELECT CO_NBR FROM NST.PARAM_COMPANY WHERE CO_NBR NOT IN (271,2996,1141)
#UNION ALL SELECT 4935 UNION ALL SELECT 5045 UNION ALL SELECT 5046 UNION ALL SELECT 5038 UNION ALL SELECT 5060";
...
//Datawarehousing processes
	...
	if($row['CURRENT_DATE']!=$row['PRN_DIG_DSH_BRD']){
		...
		GROUP BY 1"; 
		//echo "<pre>".$query."<br><br><br>";
		$result=mysql_query($query);
		$query="UPDATE CDW.PRN_DIG_DSH_BRD SET TOT_REM_ALL=(
			SELECT SUM(TOT_REM) AS TOT_REM FROM (
				SELECT 
					SUM(TOT_REM) AS TOT_REM
				FROM CMP.PRN_DIG_ORD_HEAD 
				WHERE DEL_NBR=0 AND (BUY_CO_NBR IS NULL OR BUY_CO_NBR NOT IN ($CoEx)) AND DATE(ORD_TS)='$begDate'
				UNION ALL
				SELECT 
					SUM(TOT_REM) AS TOT_REM
				FROM CMP.PRN_DIG_ORD_HEAD_ARC 
				WHERE DEL_NBR=0 AND (BUY_CO_NBR IS NULL OR BUY_CO_NBR NOT IN ($CoEx)) AND DATE(ORD_TS)='$begDate'
			)AS REM
		) WHERE DTE='$begDate'";
		...
		$query="UPDATE CDW.PRN_DIG_DSH_BRD SET TOT_REM=(SELECT SUM(TOT_REM) FROM CMP.PRN_DIG_ORD_HEAD_ARC_ALL WHERE (BUY_CO_NBR IS NULL OR BUY_CO_NBR NOT IN ($CoEx)) AND DATE(ORD_TS)='$begDate' AND DEL_NBR=0) WHERE DTE='$begDate'";
		// echo $query."<br><br>";
		mysql_query($query);
		...
		for($day=1; $day<=$days; $day++){
			...
			//echo $query."<br/>";
			$result=mysql_query($query);
			$query="UPDATE CDW.PRN_DIG_DSH_BRD SET TOT_REM_ALL=(SELECT SUM(TOT_REM) FROM CMP.PRN_DIG_ORD_HEAD_ARC_ALL WHERE DATE(ORD_TS)=DATE_ADD('$begDate',INTERVAL $day DAY) AND DEL_NBR=0) WHERE DTE=DATE_ADD('$begDate',INTERVAL $day DAY)";
			//echo $query."<br/>";
			$result=mysql_query($query);
...
#END BONUS
			$query="UPDATE CDW.PRN_DIG_DSH_BRD SET TOT_REM=(SELECT SUM(TOT_REM) FROM CMP.PRN_DIG_ORD_HEAD_ARC_ALL WHERE (BUY_CO_NBR IS NULL OR BUY_CO_NBR NOT IN ($CoEx)) AND  DATE(ORD_TS)=DATE_ADD('$begDate',INTERVAL $day DAY) AND DEL_NBR=0) WHERE DTE=DATE_ADD('$begDate',INTERVAL $day DAY)";
			echo $query."<br><br>";
			mysql_query($query);
