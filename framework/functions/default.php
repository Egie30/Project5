<?php
	...
	function genCombo($query,$key,$value,$default,$blank="")
	{
		...
		while($row=mysql_fetch_array($result))
		{
			...
			if($row[$key]==$default){echo " selected";}
			echo ">";
	...
	function genComboArrayValAssoc($array,$default,$blank="")
	{
		if($blank!="")
		{
	...
	function genComboArrayVal2($array,$default,$blank="")
	{
		if($blank!="")
		{
			echo "<option value='' selected>$blank</option>";
	...
	function parseDateTimeLiteralShort($timeStamp, $format = 'd-m-Y')
	{
        ...
        return $weekday." ".substr($timeStamp,8,2)." ".$months[strval(substr($timeStamp,5,2))-1]." ".substr($timeStamp,11,5);
	}
