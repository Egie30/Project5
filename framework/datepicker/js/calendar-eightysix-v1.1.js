...
var CalendarEightysix = new Class({
	...
	renderMonth: function() {
		...
		for(var i = (this.options.startMonday ? 1 : 0); i < (this.options.startMonday ? 8 : 7); i++) {
			var day = new Element('div', { 'style': 'background-color:#ffffff', 'html': lang.days[this.options.startMonday && i == 7 ? 0 : i] }).injectBottom(row);
	...
	switchContainers: function() {
		var oldTempContainer = this.tempContainer;
		
		this.tempContainer = this.currentContainer;
		this.currentContainer = oldTempContainer;
		
		this.currentContainer.empty().removeClass('c86-month').removeClass('c86-year-decade').setStyles({ 'visibility':'visible', 'opacity': 1, 'display': 'block', 'z-index': 999 });
