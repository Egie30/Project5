...
try {
	ob_start();
	include __DIR__ . DIRECTORY_SEPARATOR . "ajax/retail-order-report.php";
...
<table class="table-accounting tablesorter">
	<thead>
		<tr style="text-align:center">
			...
			<?php 
				if($Type == 'PAY') {
					echo "<th class='sortable'>Tgl Pembayaran</th>";
				} else {
					echo "<th class='sortable'>Tgl Selesai</th>"; 
				}
			?>
	...
	<tbody>
		...
		$linkBank="style='cursor:pointer;' onclick=".chr(34)."location.href='bank-statement-edit.php?BNK_STMT_TYP=CR&PYMT_NBR=".$data->PYMT_NBR."&ORD_NBR=".$data->ORD_NBR."&PYMT_TYP=".$data->PYMT_TYP."&ORD_TTL=".$data->ORD_TTL."&BNK_STMT_NBR=".$data->BNK_STMT_NBR."';".chr(34)." ";
			...
			<?php
				...
				if($data->BNK_STMT_DTE == null) {
					$bankStatementDate = "<td></td>";
				} else { 
					$bankStatementDate = "<td ".$linkBank.">".$bankStatementDate."</td>";
				}
...
<?php buildPagination($results->pagination, "retail-order-report-archive-ls.php"); ?>
