...
$sheet->getStyle('B5:B6')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
...
$sheet->getStyle('A16:F16')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
$sheet->setCellValue('C16', 'Size')->getStyle('C16')->getFont()->setBold(true);
$sheet->setCellValue('D16', 'Price/Rate')->getStyle('D16')->getFont()->setBold(true);
$sheet->setCellValue('E16', 'Disc')->getStyle('E16')->getFont()->setBold(true);
$sheet->setCellValue('F16', 'Total')->getStyle('F16')->getFont()->setBold(true);
...
while ($rowDetails = mysql_fetch_array($resultDetails)) {
	...
	while($rowc=mysql_fetch_array($resultc))
	{
		...
		$sheet->getStyle('D:F')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

		$sheet->setCellValue('A' . $rowIndex, $rowDetails['ORD_Q']);
		$sheet->setCellValue('B' . $rowIndex, trim($rowc['DET_TTL']." ".$rowc['PRN_DIG_DESC']));
		$sheet->setCellValue('C' . $rowIndex, trim($prnDim));
		$sheet->setCellValue('D' . $rowIndex, number_format($rowc['PRN_DIG_PRC'], 2));
		$sheet->setCellValue('E' . $rowIndex, number_format($rowc['DISC_AMT'], 2));
		$sheet->setCellValue('F' . $rowIndex, number_format($rowc['TOT_SUB'], 2));
...
$sheet->setCellValue('E' . ($rowIndex + 2), 'Subtotal');
$sheet->setCellValue('E' . ($rowIndex + 3), 'Tax');
$sheet->setCellValue('E' . ($rowIndex + 4), 'S&H');
$sheet->setCellValue('E' . ($rowIndex + 5), 'Total');
$sheet->setCellValue('E' . ($rowIndex + 6), 'Paid');
$sheet->setCellValue('E' . ($rowIndex + 7), 'Balance');

$sheet->getStyle('F' . ($rowIndex + 5))->getFont()->setBold(true); 
$sheet->getStyle('F' . ($rowIndex + 8))->getFont()->setBold(true); 

$sheet->getStyle('F' . ($rowIndex + 2) . ':F' . ($rowIndex + 8))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

$sheet->setCellValue('F' . ($rowIndex + 2), number_format($PriceTotal, 2));
$sheet->setCellValue('F' . ($rowIndex + 3), number_format($row['TAX_AMT'], 2));
$sheet->setCellValue('F' . ($rowIndex + 4), number_format($row['FEE_MISC'], 2));
$sheet->setCellValue('F' . ($rowIndex + 5), number_format($row['TOT_AMT'], 2));
$sheet->getStyle('F' . ($rowIndex + 5))->getFont()->setBold(true); 
$sheet->setCellValue('F' . ($rowIndex + 6), number_format($row['TOT_PYMT'], 2)); 
$sheet->setCellValue('F' . ($rowIndex + 7), number_format($row['TOT_REM'], 2));
$sheet->getStyle('F' . ($rowIndex + 7))->getFont()->setBold(true); 

header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header('Content-Disposition: attachment;filename="Nota #' . $orderNumber . '.xlsx"');
