								...
								if($day<=31){
									...
									*/
									$query 	= "SELECT
										...
										OT_TM_F,
										TIMESTAMPDIFF(MINUTE, CLOK_IN_TS, CLOK_OT_TS) AS JAM_ALL,
										#SUM(TIMESTAMPDIFF(MINUTE, CLOK_IN_TS, CLOK_OT_TS) - 480) AS LEMBUR_MIN,
										ALL_LEMBUR - 480 AS LEMBUR_MIN,
										(CASE WHEN TIME(CLOK_IN_TS) < '12:00:00'
											THEN  
												CASE WHEN TIMEDIFF(CLOK_IN_TS, CONCAT(DATE(CLOK_IN_TS),' ','09:00:00')) > '00:15:00'
												THEN 'OV' ELSE '' END
											  ...
											  WHEN TIME(CLOK_IN_TS) >= '18:00:00'
												...
												THEN 'OV' ELSE '' END
										END) AS OVER
									FROM PAY.MACH_CLOK_PROCESS REG
										LEFT OUTER JOIN(
											SELECT 
												PRSN_NBR,
												DATE(CLOK_IN_TS) AS CLOK_IN_DTE,
												SUM(TIMESTAMPDIFF(MINUTE, CLOK_IN_TS, CLOK_OT_TS)) AS ALL_LEMBUR
											FROM PAY.MACH_CLOK_PROCESS
											GROUP BY PRSN_NBR, DATE(CLOK_IN_TS)
										)OTM ON OTM.PRSN_NBR = REG.PRSN_NBR AND CLOK_IN_DTE = DATE(REG.CLOK_IN_TS)
									WHERE 
										REG.PRSN_NBR=".$PrsnNbr." 
										AND DAY(CLOK_IN_TS)=".$day."
										AND DATE(CLOK_IN_TS) BETWEEN '".$PayBegDte."' AND '".$PayEndDte."' 
									GROUP BY CLOK_NBR
									ORDER BY CLOK_IN_TS";
									//echo "<pre>".$query."</pre>";
