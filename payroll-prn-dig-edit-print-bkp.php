<?php
    ...
    $string .= followspace("         Gaji pokok             : Rp. " . leadSpace($row['PAY_BASE'] / $row['PYMT_DAYS'], 9) . " x " . leadspace($row['BASE_CNT'], 2) . " hari =   Rp. " . leadspace($row['BASE_TOT'], 10), 76) . leadspace("", 10) . followspace("Gaji Pokok Per Bulan", 21) . ":" . " Rp. " . leadspace($people['PAY_BASE'], 11) . "           " . chr(13) . chr(10);
    ...
    $string .= followspace("         Gaji lembur            : Rp. " . leadSpace($row['PAY_OT'], 9) . " x " . leadspace($row['OT_CNT'], 2) . " jam  =   Rp. " . leadspace($row['OT_TOT'], 10), 76) . leadspace("", 10) .followspace("Gaji Lain", 21) . ":" . " Rp. " . leadspace($people['PAY_MISC'], 11) . "            " . chr(13) . chr(10);
    $string .= followspace("         Gaji tambahan          : Rp. " . leadSpace($row['PAY_ADD'] / $row['PYMT_DAYS'], 9) . " x " . leadspace($row['ADD_CNT'], 2) . " hari =   Rp. " . leadspace($row['ADD_TOT'], 10), 76) . leadspace("", 10) . followspace("Gaji Kontribusi", 21) . ":" . " Rp. " . leadspace($row['CONTRB_AMT'], 11) . "            " . chr(13) . chr(10);
	$string .= followspace("         Transportasi           : Rp. " . leadSpace($row['AUTH_TRVL_AMT']/$row['TOT_DIST'], 9) . " x " . leadspace($row['TOT_DIST'], 2) . " km   =   Rp. " . leadspace($row['AUTH_TRVL_AMT'], 10), 76) . leadspace("", 10) . followspace("Potongan Bon/Bulan", 21) . ":" . " Rp. " . leadspace($row['DED_DEF_PRSN'], 11) . "            " .  chr(13) . chr(10);
	if ($row['STY_CNT'] == 0){$menginap = 0;}else{$menginap = $row['STY_TOT_AMT'] / $row['STY_CNT'];}
	$string .= followspace("         Menginap               : Rp. " . leadSpace($menginap, 9) . " x " . leadspace($row['STY_CNT'], 2) . " hari =   Rp. " . leadspace($row['STY_TOT_AMT'], 10), 76) . leadspace("", 10) . followspace("Bonus", 21) . ":" . " Rp. " . leadspace($people['BONUS'], 11) . "            " .  chr(13) . chr(10);
	...
	$string .= followspace("                                                 Jumlah   =   Rp. " . leadSpace($row['BASE_TOT'] + $row['BON_MO_AMT']+ $row['CMSN_AMT'] + $row['OT_TOT'] + $row['ADD_TOT'] + $row['AUTH_TRVL_AMT'] + $row['STY_TOT_AMT'], 10), 76) . leadspace("", 10) . followspace("Sisa Bon", 21) . ":" . " Rp. " . leadspace($people['AV_CRD'], 11).chr(13) . chr(10);
