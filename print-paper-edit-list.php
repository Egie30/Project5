<?php
	...
	if($type == "EST"){
		...
		$tableAdd	= "CMP.PRN_PPR_ORD_VAL_ADD_EST";
	}else{
		$headtable 	= "CMP.PRN_PPR_ORD_HEAD";
		$detailtable= "CMP.PRN_PPR_ORD_DET";
		$tableAdd	= "CMP.PRN_PPR_ORD_VAL_ADD";
	}
...
<table style="background:#ffffff;">
	...
	<?php
		$query="SELECT ORD_DET_NBR,DET.ORD_NBR,DET_TTL,PRN_PPR_DESC,DET.PRN_PPR_PRC,ORD_Q,FIL_LOC,PRN_LEN,PRN_WID,FEE_MISC,FAIL_CNT,DISC_PCT,DISC_AMT,VAL_ADD_AMT,TOT_SUB,ROLL_F,HND_OFF_TYP,HND_OFF_TS,SORT_BAY_ID,TYP.PRN_PPR_CD,EQP.PRN_PPR_EQP_COLR, DET.PRN_PPR_TYP
				FROM ". $detailtable ." DET LEFT OUTER JOIN
				     CMP.PRN_PPR_TYP TYP ON DET.PRN_PPR_TYP=TYP.PRN_PPR_TYP LEFT OUTER JOIN
					 CMP.PRN_PPR_EQP EQP ON TYP.PRN_PPR_EQP=EQP.PRN_PPR_EQP
		...
		while($rowd=mysql_fetch_array($result)){
            ...
            $TotSub		= 0;
            $rowDetail	= '';
            $typeCode[]	= $rowd['PRN_PPR_CD'];

			$query="SELECT ORD_DET_NBR,DET.ORD_NBR,ORD_DET_NBR_PAR,DET_TTL,TYP.PRN_PPR_TYP,PRN_PPR_DESC,DET.PRN_PPR_PRC,ORD_Q,FIL_LOC,PRN_LEN,PRN_WID,FEE_MISC,FAIL_CNT,DISC_PCT,DISC_AMT,VAL_ADD_AMT,TOT_SUB,ROLL_F,HND_OFF_TYP,HND_OFF_TS,SORT_BAY_ID,PPL.NAME,DET.UPD_TS,DET.N_UP, DET.PROD_NBR,TYP.PRN_PPR_CD,EQP.PRN_PPR_EQP_COLR
					FROM ". $detailtable ." DET LEFT OUTER JOIN
						     CMP.PRN_PPR_TYP TYP ON DET.PRN_PPR_TYP=TYP.PRN_PPR_TYP LEFT OUTER JOIN
						 CMP.PRN_PPR_EQP EQP ON TYP.PRN_PPR_EQP=EQP.PRN_PPR_EQP
			...
			while($rowc=mysql_fetch_array($resultc)){
				...
				if ($rowc['PROD_NBR']!=''){
					$link = "print-paper-edit-list-product-detail.php";
				}else{
					$link = "print-paper-edit-list-detail-2.php";
				}

				If($security==0){
					$rowDetail.="onclick=".chr(34)."slideFormIn('".$link."?ORD_DET_NBR=".$rowc['ORD_DET_NBR']."&ORD_NBR=".$OrdNbr."&TYP=".$type."&SHOW=".$Show."&STT=".$_GET['STT']."&ORGN=".$origin."');".chr(34);
				}			
				if(($security==1)&&(in_array($row["ORD_STT_ID"],array('NE','RC','QU','PF')))){
					$rowDetail.="onclick=".chr(34)."slideFormIn('".$link."?ORD_DET_NBR=".$rowc['ORD_DET_NBR']."&ORD_NBR=".$OrdNbr."&TYP=".$type."&SHOW=".$Show."&STT=".$_GET['STT']."&ORGN=".$origin."');".chr(34);
				}
				if(($security==1)&&(in_array($row["ORD_STT_ID"],array('RD','DL','CP')))){
				    $rowDetail.="onclick=".chr(34)."slideFormIn('".$link."?ORD_DET_NBR=".$rowc['ORD_DET_NBR']."&ORD_NBR=".$OrdNbr."&TYP=".$type."&readonly=1&SHOW=".$Show."&STT=".$_GET['STT']."&ORGN=".$origin."');".chr(34);
				}
				...
				$rowDetail.=">".$OpenS.trim($rowc['DET_TTL']." ".$rowc['PRN_PPR_DESC']).$prnDim.$nUp.$CloseS."</td>";
				...
				$rowDetail.="</td>";
                if(($rowc['ORD_DET_NBR_PAR']=='')&&($rowc['PRN_PPR_TYP']=='PROD')){
                    $OrdQ=$rowc['ORD_Q'];
                }
                $PrnDigPrc+=$rowc['PRN_PPR_PRC'];
                ...
                if(($rowc['ORD_DET_NBR_PAR']=='')&&($rowc['PRN_PPR_TYP']=='PROD')){
				    ...
				    $rowDetail.="<td style='cursor:pointer;text-align:right;'>[PRN_PPR_PRC]</td>";
					...
					if($security != 7 && $UpperSec != 8){
                    $rowDetail.="<td style='cursor:pointer;text-align:right;$color'>".$OpenS.number_format($rowc['PRN_PPR_PRC'],0,",",".").$CloseS."</td>";
				...
				If($security==0){
					if($rowc['ORD_DET_NBR_PAR']==""){
						$rowDetail.="<div class='listable-btn'><span class='fa fa-list-ul listable-btn' onclick=".chr(34)."event.cancelBubble=true;slideFormIn('print-paper-edit-list-detail-2.php?ORD_DET_NBR=0&ORD_NBR=".$OrdNbr."&ORD_DET_NBR_PAR=".$rowc['ORD_DET_NBR']."&TYP=".$type."');".chr(34)."></span></div>";
					}else{
						$rowDetail.="<img class='listable' src='img/blank16x16.png'>";
					}
					$rowDetail.="<div class='listable-btn'><span class='fa fa-trash listable-btn'  onclick=".chr(34)."event.cancelBubble=true;syncGetContent('edit-list','print-digital-edit-list.php?ORD_NBR=".$OrdNbr."&DEL_D=".$rowc['ORD_DET_NBR']."&TYP=".$type."');syncGetContent('pay','print-digital-payment-list.php?ORD_NBR=".$OrdNbr."&TYP=".$type."');calcAmt();".chr(34)."></span></div>";
				}
				if(($security==1)&&(in_array($row["ORD_STT_ID"],array('NE','RC','QU')))){
					if($rowc['ORD_DET_NBR_PAR']==""){
						$rowDetail.="<div class='listable-btn'><span class='fa fa-list-ul listable-btn' onclick=".chr(34)."event.cancelBubble=true;slideFormIn('print-paper-edit-list-detail-2.php?ORD_DET_NBR=0&ORD_NBR=".$OrdNbr."&ORD_DET_NBR_PAR=".$rowc['ORD_DET_NBR']."&TYP=".$type."');".chr(34)."></span></div>";
					...
					$rowDetail.="<div class='listable-btn'><span class='fa fa-trash listable-btn'  onclick=".chr(34)."event.cancelBubble=true;getContent('edit-list','print-digital-edit-list.php?ORD_NBR=".$OrdNbr."&DEL_D=".$rowc['ORD_DET_NBR']."&TYP=".$type."');syncGetContent('pay','print-digital-payment-list.php?ORD_NBR=".$OrdNbr."&TYP=".$type."');calcAmt();".chr(34)."></span></div>";
				}
				...
				$query="SELECT ORD_VAL_ADD_NBR,VAL_ADD_Q,VAL_ADD_DESC,VAD.VAL_ADD_PRC,FEE_MISC,DISC_PCT,DISC_AMT,TOT_SUB
						 FROM ".$tableAdd." VAD LEFT OUTER JOIN
						      CMP.PRN_PPR_VAL_ADD_TYP TYP ON VAD.VAL_ADD_TYP=TYP.VAL_ADD_TYP
				...
				while($rowa=mysql_fetch_array($resulta)){
					...
					$rowDetail.="<img class='listable' src='img/trash.png' onclick=".chr(34)."syncGetContent('edit-list','print-digital-edit-list.php?ORD_NBR=".$OrdNbr."&DEL_A=".$rowa['ORD_VAL_ADD_NBR']."');syncGetContent('pay','print-digital-payment-list.php?ORD_NBR=".$OrdNbr."&TYP=".$type."');calcAmt();".chr(34).">";
					$TotNet+=$rowa['TOT_SUB'];
					$rowDetail.="</td></tr>";
				}
            ...
            if($OrdQ!=''){
                ...
                $rowDetail=str_replace("[PRN_PPR_PRC]",$PrnDigPrc,$rowDetail);
