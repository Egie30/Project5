<?php
	...
	$query="SELECT THD.TRNSP_NBR,OHD.ORD_NBR,THD.ORD_TTL,BUY_PRSN_NBR,BUY_CO_NBR,PPL.NAME AS NAME_PPL,BCM.NAME AS NAME_CO,THD.DUE_TS,TRNSP_STT_DESC,JOB_LEN_TOT,COUNT(*) AS TYPE_CNT,SUM(TRNSP_Q) AS ITEM_CNT
            ...
            WHERE THD.DEL_NBR=0 ".$where." AND (DET.DEL_NBR=0 OR DET.DEL_NBR IS NULL) AND (THD.ORD_TTL LIKE '%".$searchQuery."%' OR PPL.NAME LIKE '%".$searchQuery."%' OR BCM.NAME LIKE '%".$searchQuery."%' OR THD.ORD_NBR LIKE '%".$searchQuery."%' OR THD.TRNSP_NBR LIKE '%".$searchQuery."%')
