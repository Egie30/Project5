...
$CoEx	= "SELECT CO_NBR FROM NST.PARAM_COMPANY";
	...
	// $row['CURRENT_DATE']!=$row['PRN_DIG_DSH_BRD']
	if($row['CURRENT_DATE']!=$row['PRN_DIG_DSH_BRD']){
		...
		$query_date	= "SELECT DATE(MIN(ORD_TS)) AS DEL_DTE FROM (
						...
						UNION 
							SELECT 
								...
								ORD_TS, 
								UPD_TS 
							FROM CMP.PRN_DIG_ORD_HEAD_ARC_ALL
							WHERE UPD_TS >= (NOW() - INTERVAL 1 MONTH) 
							ORDER BY ORD_TS
						) T1";
						echo "<pre>".$query_date;
		...
		$query="INSERT INTO CDW.PRN_DIG_DSH_BRD (DTE,REV_ALL,TOT_REM_ALL,FLJ320P_ALL,KMC6501_ALL,RVS640_ALL,KMC8000_ALL,KMC1085_ALL,HPL375_ALL,ATX67_ALL,LABSVCS_ALL,REV_FLJ320P_ALL,REV_KMC6501_ALL,REV_RVS640_ALL,AJ1800F_ALL,REV_AJ1800F_ALL,MVJ1624_ALL,REV_MVJ1624_ALL,REV_KMC8000_ALL,REV_KMC1085_ALL,REV_HPL375_ALL,REV_ATX67_ALL,REV_LABSVCS_ALL) 
				...
				FROM (
				SELECT 
					DATE(ORD_TS) AS ORDER_DATE,
					SUM(TOT_SUB) + COALESCE(SUM(TOT_SUB_ADD), 0) AS TOTAL,
					0 AS DUMMY_COLUMN,
				...
				LEFT OUTER JOIN 
								(SELECT ORD_DET_NBR,SUM(TOT_SUB) AS TOT_SUB_ADD 
									FROM CMP.PRN_DIG_ORD_VAL_ADD
									GROUP BY ORD_NBR) VAL ON DET.ORD_DET_NBR=VAL.ORD_DET_NBR
				WHERE (BUY_CO_NBR IS NULL OR BUY_CO_NBR NOT IN ($CoEx)) 
					AND DATE(ORD_TS)='$begDate' 
					AND HED.DEL_NBR=0 
					AND DET.DEL_NBR=0
				GROUP BY 1
			UNION 	
				SELECT 
					...
					COALESCE(SUM(CASE WHEN PRN_DIG_EQP='ATX67' THEN TOT_SUB ELSE 0 END),0) AS REV_ATX67,
					COALESCE(SUM(CASE WHEN PRN_DIG_EQP='LABSVCS' THEN TOT_SUB ELSE 0 END),0) AS REV_LABSVCS
				FROM CMP.PRN_DIG_ORD_HEAD_ARC ARC 
				LEFT OUTER JOIN CMP.PRN_DIG_ORD_DET_ARC DETARC ON ARC.ORD_NBR=DETARC.ORD_NBR 
				LEFT OUTER JOIN CMP.PRN_DIG_TYP TYP ON DETARC.PRN_DIG_TYP=TYP.PRN_DIG_TYP 
				LEFT OUTER JOIN 
								(SELECT ORD_DET_NBR,SUM(TOT_SUB) AS TOT_SUB_ADD 
									FROM CMP.PRN_DIG_ORD_VAL_ADD
									GROUP BY ORD_NBR) VAL ON DETARC.ORD_DET_NBR=VAL.ORD_DET_NBR
				WHERE (BUY_CO_NBR IS NULL OR BUY_CO_NBR NOT IN ($CoEx))  
					AND DATE(ORD_TS)='$begDate' 
					AND ARC.DEL_NBR=0 
					AND DETARC.DEL_NBR=0
				GROUP BY 1
			UNION 
				SELECT 
					DATE(ORD_TS) AS ORDER_DATE,
					...
					COALESCE(SUM(CASE WHEN PRN_DIG_EQP='ATX67' THEN TOT_SUB ELSE 0 END),0) AS REV_ATX67,
					COALESCE(SUM(CASE WHEN PRN_DIG_EQP='LABSVCS' THEN TOT_SUB ELSE 0 END),0) AS REV_LABSVCS
				FROM CMP.PRN_DIG_ORD_HEAD_ARC_ALL HEDALL 
				LEFT OUTER JOIN CMP.PRN_DIG_ORD_DET_ARC_ALL DETALL ON HEDALL.ORD_NBR=DETALL.ORD_NBR 
				LEFT OUTER JOIN CMP.PRN_DIG_TYP TYP ON DETALL.PRN_DIG_TYP=TYP.PRN_DIG_TYP 
				LEFT OUTER JOIN 
								(SELECT ORD_DET_NBR,SUM(TOT_SUB) AS TOT_SUB_ADD 
									FROM CMP.PRN_DIG_ORD_VAL_ADD
									GROUP BY ORD_NBR) VAL ON DETALL.ORD_DET_NBR=VAL.ORD_DET_NBR
				WHERE (BUY_CO_NBR IS NULL OR BUY_CO_NBR NOT IN ($CoEx))  
					AND DATE(ORD_TS)='$begDate' 
					AND HEDALL.DEL_NBR=0 
					AND DETALL.DEL_NBR=0
				GROUP BY 1
				)AS T01
				GROUP BY ORDER_DATE"; 
		...
		$query="UPDATE CDW.PRN_DIG_DSH_BRD 
				SET 
				TOT_REM_ALL = 	(SELECT 
									COALESCE(SUM(TOT_REM), 0) +
									COALESCE((SELECT SUM(TOT_REM) 
											FROM CMP.PRN_DIG_ORD_HEAD_ARC 
											WHERE DATE(ORD_TS) = '$begDate' AND DEL_NBR = 0), 0) +
									COALESCE((SELECT SUM(TOT_REM) 
											FROM CMP.PRN_DIG_ORD_HEAD_ARC_ALL 
											WHERE DATE(ORD_TS) = '$begDate' AND DEL_NBR = 0 
											GROUP BY DATE(ORD_TS)), 0)
								FROM CMP.PRN_DIG_ORD_HEAD 
								WHERE DATE(ORD_TS) = '$begDate' AND DEL_NBR = 0
								)
				WHERE DTE = '$begDate'";
		...
		$query="UPDATE CDW.PRN_DIG_DSH_BRD DSB 
				LEFT OUTER JOIN (
					SELECT ORD_TS,
					SUM(TOT_SUB)+COALESCE(SUM(TOT_SUB_ADD),0) REV,
					SUM(CASE WHEN PRN_DIG_EQP='FLJ320P' THEN ORD_Q*PRN_LEN*PRN_WID ELSE 0 END) AS FLJ320P,
					SUM(CASE WHEN PRN_DIG_EQP='KMC6501' THEN ORD_Q ELSE 0 END) AS KMC6501,
					SUM(CASE WHEN PRN_DIG_EQP='RVS640' THEN ORD_Q*PRN_LEN*PRN_WID ELSE 0 END) AS RVS640,
					COALESCE(SUM(CASE WHEN PRN_DIG_EQP='FLJ320P' THEN TOT_SUB ELSE 0 END),0) AS REV_FLJ320P,
			...
			UNION 
				...
				SET DSB.REV=ORD.REV,
					...
					DSB.REV_MVJ1624=ORD.REV_MVJ1624 
		...
		$query="UPDATE CDW.PRN_DIG_DSH_BRD 
				SET TOT_REM = 	( SELECT 
									...
									COALESCE((SELECT SUM(TOT_REM) 
												FROM CMP.PRN_DIG_ORD_HEAD_ARC 
												WHERE (BUY_CO_NBR IS NULL OR BUY_CO_NBR NOT IN ($CoEx)) 
														AND DATE(ORD_TS) = '$begDate' AND DEL_NBR = 0), 0) +
									COALESCE((SELECT SUM(TOT_REM) 
												FROM CMP.PRN_DIG_ORD_HEAD_ARC_ALL 
												WHERE (BUY_CO_NBR IS NULL OR BUY_CO_NBR NOT IN ($CoEx)) 
														AND DATE(ORD_TS) = '$begDate' AND DEL_NBR = 0), 0)
								FROM CMP.PRN_DIG_ORD_HEAD 
								WHERE (BUY_CO_NBR IS NULL OR BUY_CO_NBR NOT IN ($CoEx)) 
									AND DATE(ORD_TS) = '$begDate' AND DEL_NBR = 0
								)
				WHERE DTE = '$begDate'";
		...
		$query = "UPDATE CDW.PRN_DIG_DSH_BRD SET REV_RETAIL=(
					...
					FROM RTL.CSH_REG CSH 
					LEFT OUTER JOIN (SELECT INV_BCD, 
											CAT_NBR 
									FROM RTL.INVENTORY 
									WHERE DEL_NBR=0 
		...
		//Retail sales
		$query="UPDATE CDW.PRN_DIG_DSH_BRD DSB 
				LEFT OUTER JOIN (SELECT ORD_DTE,
										SUM(TOT_SUB) AS REV
								FROM RTL.RTL_ORD_HEAD HED 
								LEFT JOIN RTL.RTL_ORD_DET DET ON HED.ORD_NBR=DET.ORD_NBR 
		...
		$query="UPDATE CDW.PRN_DIG_DSH_BRD 
					SET TOT_REM_ORD=(SELECT SUM(TOT_REM) 
									FROM RTL.RTL_ORD_HEAD 
									WHERE (RCV_CO_NBR IS NULL OR RCV_CO_NBR NOT IN ($CoEx)) 
										AND ORD_DTE='$begDate' 
										AND DEL_F=0	) 
		...
		for($day=1; $day<=$days; $day++){
			...
			if(mysql_num_rows($result)==0){
				...
				$query="INSERT INTO CDW.PRN_DIG_DSH_BRD (DTE,REV_ALL,TOT_REM_ALL,FLJ320P_ALL,KMC6501_ALL,RVS640_ALL,KMC8000_ALL,KMC1085_ALL,HPL375_ALL,ATX67_ALL,LABSVCS_ALL,REV_FLJ320P_ALL,REV_KMC6501_ALL,REV_RVS640_ALL,AJ1800F_ALL,REV_AJ1800F_ALL,MVJ1624_ALL,REV_MVJ1624_ALL,REV_KMC8000_ALL,REV_KMC1085_ALL,REV_HPL375_ALL,REV_ATX67_ALL,REV_LABSVCS_ALL) 
						...
						LEFT OUTER JOIN CMP.PRN_DIG_ORD_DET DET ON HED.ORD_NBR=DET.ORD_NBR 
						LEFT OUTER JOIN CMP.PRN_DIG_TYP TYP ON DET.PRN_DIG_TYP=TYP.PRN_DIG_TYP 
						LEFT OUTER JOIN (SELECT ORD_DET_NBR,SUM(TOT_SUB) AS TOT_SUB_ADD 
										FROM CMP.PRN_DIG_ORD_VAL_ADD
										GROUP BY ORD_NBR) VAL ON DET.ORD_DET_NBR=VAL.ORD_DET_NBR
						WHERE (BUY_CO_NBR IS NULL OR BUY_CO_NBR NOT IN ($CoEx))  
							AND DATE(ORD_TS)=DATE_ADD('$begDate',INTERVAL $day DAY) 
							AND HED.DEL_NBR=0 
							AND DET.DEL_NBR=0
						GROUP BY 1
					UNION 	
			...
			$query="UPDATE CDW.PRN_DIG_DSH_BRD 
					SET TOT_REM_ALL = (
						SELECT COALESCE(SUM(TOT_REM), 0) +
							COALESCE((SELECT SUM(TOT_REM) 
										FROM CMP.PRN_DIG_ORD_HEAD_ARC 
										WHERE DATE(ORD_TS) = DATE_ADD('$begDate', INTERVAL $day DAY) AND DEL_NBR = 0), 0) +
							COALESCE((SELECT SUM(TOT_REM) 
										FROM CMP.PRN_DIG_ORD_HEAD_ARC_ALL 
										WHERE DATE(ORD_TS) = DATE_ADD('$begDate', INTERVAL $day DAY) AND DEL_NBR = 0), 0)
						FROM CMP.PRN_DIG_ORD_HEAD 
			...
			$query="UPDATE CDW.PRN_DIG_DSH_BRD DSB 
					...
					UNION 
						...
						WHERE (BUY_CO_NBR IS NULL OR BUY_CO_NBR NOT IN ($CoEx)) 
							AND DATE(ORD_TS)=DATE_ADD('$begDate',INTERVAL $day DAY) 
							AND HEDALL.DEL_NBR=0 
							AND DETALL.DEL_NBR=0) ORD ON DATE(ORD.ORD_TS)=DATE_ADD('$begDate',INTERVAL $day DAY)
					SET 
						DSB.REV=ORD.REV,
						DSB.FLJ320P=ORD.FLJ320P,
						DSB.KMC6501=ORD.KMC6501,
