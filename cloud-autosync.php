...
if(cloudCheck()==true){
	if(serverCheck($local)==true){
        ...
        while($row=mysql_fetch_array($result)){
            ...
            if($row['SYNC_DIR_TYP']=='2'){
                $data.=date('Y-m-d').' '.gmdate("H:i:s",time()+7*3600).' Checking '.$row['DB_NM'].'.'.$row['TBL_NM']." for 2D sync\n";
                $j=syncTable($row['TBL_NM'],$row['COL_NM'],$row['DB_NM'],$curDB,$local,$cloud);
