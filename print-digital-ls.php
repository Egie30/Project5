<?php
	...
	if($OrdSttId=="ALL"){
        ...
        $where="WHERE HED.ORD_STT_ID!='CP' AND (HED.ORD_NBR IN (SELECT ORD_NBR FROM CMP.JRN_PRN_DIG WHERE CRT_NBR IN (".$children.") GROUP BY ORD_NBR) OR CRT_NBR IN (".$_SESSION['personNBR'].$children.") OR SLS_PRSN_NBR IN (".$children.") OR ACCT_EXEC_NBR IN (".$children.")) AND HED.DEL_NBR=0";
	}elseif($OrdSttId=="ACT"){
		$where="WHERE (HED.ORD_STT_ID!='CP' OR (HED.ORD_STT_ID='CP' AND TIMESTAMPADD(MONTH,$activePeriod,ORD_TS)>=CURRENT_TIMESTAMP AND LAST_DAY(DATE_ADD(ORD_TS,INTERVAL COALESCE(COM.PAY_TERM,32) DAY))>=CURRENT_DATE) OR (HED.ORD_STT_ID!='CP' AND TOT_REM>0 AND TIMESTAMPADD(MONTH,$badPeriod,ORD_TS)>=CURRENT_TIMESTAMP)) AND HED.DEL_NBR=0";
	}elseif($OrdSttId=="SLM"){
		$where="WHERE HED.ORD_STT_ID!='CP' AND JRN.CRT_TS <= NOW() - INTERVAL 48 HOUR AND HED.DEL_NBR=0";
	}elseif($OrdSttId=="CP"){
		$where="WHERE HED.ORD_STT_ID='CP' AND TIMESTAMPADD(MONTH,$activePeriod,ORD_TS)>=CURRENT_TIMESTAMP AND HED.DEL_NBR=0";
	}elseif($OrdSttId=="DUE"){
		$where="WHERE TOT_REM>0 AND DATE_ADD(CMP_TS,INTERVAL COALESCE(COM.PAY_TERM,0) DAY)<=CURRENT_TIMESTAMP AND HED.DEL_NBR=0";
	}elseif($OrdSttId=="COL"){
		...
		$where="WHERE HED.ORD_STT_ID ='CP' AND TOT_REM>0 AND LAST_DAY(DATE_ADD(ORD_TS,INTERVAL COALESCE(COM.PAY_TERM,32) DAY))<=CURRENT_DATE AND HED.DEL_NBR=0";
	}else{
	...
	$query="SELECT HED.ORD_NBR,NBR,DL_CNT,PU_CNT,NS_CNT,IVC_PRN_CNT,ORD_TS,HED.ORD_STT_ID,ORD_STT_DESC,BUY_PRSN_NBR,PPL.NAME AS NAME_PPL,COM.NAME AS NAME_CO,BUY_CO_NBR,REF_NBR,ORD_TTL,DUE_TS,JOB_LEN_TOT,PRN_CO_NBR,FEE_MISC,TOT_AMT,PYMT_DOWN,PYMT_REM,TOT_REM,CMP_TS,PU_TS,SPC_NTE,HED.CRT_TS,HED.CRT_NBR,HED.UPD_TS,HED.UPD_NBR,CMP_TS,
	TIMESTAMPDIFF(HOUR,JRN.CRT_TS,NOW()) AS SLM_HRS,
	DATEDIFF(DATE_ADD(CMP_TS,INTERVAL COALESCE(COM.PAY_TERM,0) DAY),CURRENT_TIMESTAMP) AS PAST_DUE,HED.PRN_DIG_CD,HED.PRN_DIG_EQP_COLR
			FROM ". $headtable ." HED
			...
			$where AND (ORD_TTL LIKE '%".$searchQuery."%' OR PPL.NAME LIKE '%".$searchQuery."%' OR COM.NAME LIKE '%".$searchQuery."%' OR HED.ORD_NBR LIKE '%".$searchQuery."%')
			AND HED.DEL_NBR=0
	...
	while($row=mysql_fetch_array($result))
	{
		...
		echo "<div style='margin-top:2px;'>";
			...
			foreach( $items as $data => $item){
				$color = $colors[$data];
				if($item != ''){
					echo "<span style='padding: 1px 3px 1px 3px;
