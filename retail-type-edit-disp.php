<?php
	...
	$whereClause.="STA.STA_NBR=".$_GET['RTL_NBR']." ";
	//Search for inventory number
	$query="SELECT STA.STA_NBR,PRC,CONCAT(STA.NAME,' ',COLR_DESC,' ',MATR,' ',SIZE,' ',TYPE) AS NAME_DESC,CMP.NAME,SUM(CASE WHEN MOV_TYP='IN' THEN MOV_CNT ELSE -1*MOV_CNT END) AS QTY
	...
	if(mysql_num_rows($result)>0)
	{
		...
		while($row=mysql_fetch_array($result))
		{
			...
			echo "<tr $alt style='cursor:pointer;' onclick=".chr(34)."document.getElementById('RTL_NBR').value=".$row['STA_NBR'].chr(34).">";
