<?php
	...
	$query="SELECT STA.STA_NBR,CONCAT(STA.NAME,' ',COLR_DESC,' ',MATR,' ',SIZE,' ',TYPE) AS NAME_DESC,
			...
			WHERE STA.CO_NBR=".$CoNbr."
			GROUP BY STA.STA_NBR
	...
	while($row=mysql_fetch_array($result))
	{
		if($title!=strtoupper(substr($row['NAME_DESC'],0,1)))
		{
			...
			echo "<tr class='listable'><td class='listable' colspan='18' style='padding-top:10px;border-left:1px'><strong>".$title."</strong></td></tr>";
			...
			echo "<th class='llistable'>".sprintf("%02d",date("d",time()-86400*4))."</th>";
		...
		echo "<td style='align:center'>".$row['STA_NBR']."</td>";
	...
	function dispInv($ref,$val)
	{
		if($val==0){
			...
			echo "<td class='list' align='right'><a href='stationery-edit.php?LOG_NBR=".$ref."'>".number_format($val,0,",",".")."</a></td>";
		}
	}
?>
