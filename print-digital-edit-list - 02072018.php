...
<table style="background:#ffffff;">
	<tr>
		...
		<?php } ?>
		<th class="listable"><?php if($OrdNbr!=0){echo $detailAdd;} ?></th>
	</tr>
	<?php
		$query="SELECT ORD_DET_NBR,DET.ORD_NBR,DET_TTL,PRN_DIG_DESC,DET.PRN_DIG_PRC,ORD_Q,FIL_LOC,PRN_LEN,PRN_WID,FEE_MISC,FAIL_CNT,DISC_PCT,DISC_AMT,VAL_ADD_AMT,TOT_SUB,ROLL_F,HND_OFF_TYP,HND_OFF_TS,SORT_BAY_ID
		...
		while($rowd=mysql_fetch_array($result))
		{
            ...
            $TotSub=0;
            $rowDetail='';
			$query="SELECT ORD_DET_NBR,DET.ORD_NBR,ORD_DET_NBR_PAR,DET_TTL,TYP.PRN_DIG_TYP,PRN_DIG_DESC,DET.PRN_DIG_PRC,ORD_Q,FIL_LOC,PRN_LEN,PRN_WID,FEE_MISC,FAIL_CNT,DISC_PCT,DISC_AMT,VAL_ADD_AMT,TOT_SUB,ROLL_F,HND_OFF_TYP,HND_OFF_TS,SORT_BAY_ID,PPL.NAME,DET.UPD_TS,DET.N_UP
					FROM ". $detailtable ." DET LEFT OUTER JOIN
						     CMP.PRN_DIG_TYP TYP ON DET.PRN_DIG_TYP=TYP.PRN_DIG_TYP LEFT OUTER JOIN
						 CMP.PRN_DIG_EQP EQP ON TYP.PRN_DIG_EQP=EQP.PRN_DIG_EQP
			...
			while($rowc=mysql_fetch_array($resultc))
			{
				if ($Show=="NO") 
					...
					{ $rowDetail.="<tr $alt "; }
				If($security==0){
					$rowDetail.="onclick=".chr(34)."slideFormIn('print-digital-edit-list-detail.php?ORD_DET_NBR=".$rowc['ORD_DET_NBR']."&ORD_NBR=".$OrdNbr."&TYP=".$type."&SHOW=".$Show."&STT=".$_GET['STT']."');".chr(34);
				}			
				if(($security==1)&&(in_array($row["ORD_STT_ID"],array('NE','RC','QU','PF')))){
					$rowDetail.="onclick=".chr(34)."slideFormIn('print-digital-edit-list-detail.php?ORD_DET_NBR=".$rowc['ORD_DET_NBR']."&ORD_NBR=".$OrdNbr."&TYP=".$type."&SHOW=".$Show."&STT=".$_GET['STT']."');".chr(34);
				}
				if(($security==1)&&(in_array($row["ORD_STT_ID"],array('RD','DL','CP')))){
				    $rowDetail.="onclick=".chr(34)."slideFormIn('print-digital-edit-list-detail.php?ORD_DET_NBR=".$rowc['ORD_DET_NBR']."&ORD_NBR=".$OrdNbr."&TYP=".$type."&readonly=1&SHOW=".$Show."&STT=".$_GET['STT']."');".chr(34);
				}
				$rowDetail.=">";
