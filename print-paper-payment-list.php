<?php
	...
	if($type == "EST"){
		...
		$paymenttable	= "CMP.PRN_PPR_ORD_PYMT_EST";
	}else{
		$headtable 		= "CMP.PRN_PPR_ORD_HEAD";
		$paymenttable	= "CMP.PRN_PPR_ORD_PYMT";
	}
	...
	if($TndAmt!=''){
		//Add Payment
		$query="INSERT INTO ". $paymenttable ." SET ORD_NBR=$OrdNbr,TND_AMT=$TndAmt,DEL_NBR=0,CRT_TS=CURRENT_TIMESTAMP,CRT_NBR=$PrsnNbr";
		$result=mysql_query($query);
	}
	if($TndAmt!=''){
	...
	$query="SELECT 
		...
		REF, 
		PPL.NAME, 
		DATE_FORMAT(PYM.CRT_TS,'%d-%m-%Y') AS DTE, 
		PYM.CRT_NBR
	...
	while($row=mysql_fetch_array($result)){
		...
		echo ">";
		echo "<td style='padding-left:7px;width:300px;padding-top:1px;'>";
		echo "<div class='print-digital-grey'>".$row['PYMT_NBR']."</div>";
		...
		echo "<div class='listable-btn' style='margin-left:5px'>
			<span class='fa fa-trash listable-btn' onclick=".chr(34)."syncGetContent('pay','print-digital-payment-list.php?ORD_NBR=".$OrdNbr."&PYMT_NBR=".$row['PYMT_NBR']."&TYP=".$type."&PYMT_TYP=DEL');calcAmt();".chr(34)."></span>
		</div>";
		}
		$TotNet+=$rowa['TOT_SUB'];
...
<script>
	...
	<tr class='total'>
		...
		<td style='width:30px;'>
			<div id='converse' class='listable-btn' style='margin-left:5px'><span class='fa fa-plus listable-btn' onclick="syncGetContent('pay','print-digital-payment-list.php?TND_AMT='+document.getElementById('TND_AMT').value+'&ORD_NBR=<?php echo $OrdNbr; ?>&TYP=<?php echo $type; ?>');calcAmt();" ></span></div>
		</td>
	</tr>
</table>