    ...
    <body>
        <?php
            ...
            LEFT JOIN ( 
                ...
                WHERE DATE(HED.ORD_TS) = '$start_date'
            ...
            WHERE DET.ORD_NBR != 0
                AND DET.DEL_NBR= 0
                AND (HED.ORD_TTL LIKE '%meeting room%' OR HED.ORD_TTL LIKE '%coworking%')"; 
        ...
        <div id="mainResult" class="printDigitalPopupEditContent">
            <table style="width: 100%">
                <tbody style="line-height: 10px;">
                    ...
                    <?php
                $desk = '';
                $firstClassAdded = false; // Tambahkan variabel untuk menandai apakah class "first" sudah ditambahkan
                while ($row = mysql_fetch_assoc($show)) {
                    $DESC = createDescription($row);
                    if ($desk != $row['ORD_NBR']) {
                        if($firstClassAdded) { // Tambahkan class "first" jika sudah ditambahkan sebelumnya
                            echo '<tr><td class="first"></td></tr>';
                            $firstClassAdded = false; // Setel kembali ke false untuk persiapan item selanjutnya
                        }
                        ?>
                    ...
                    <tr data-order="<?php echo $row['ORD_NBR']; ?>">
                        <td style="text-align:left;white-space:nowrap;float:left;">Mulai: <?php echo $row['ORD_TS']; ?></td>
                    </tr>
                    <tr data-order="<?php echo $row['ORD_NBR']; ?>">
                    ...
                    <?php
                    $desk = $row['ORD_NBR'];
                    $firstClassAdded = true; // Tandai bahwa class "first" telah ditambahkan setelah item selesai
                }
                if($firstClassAdded) { // Jika class "first" belum ditambahkan untuk item terakhir
                    echo '<tr><td class="first"></td></tr>';
                }
                ?>
                        </tr>
