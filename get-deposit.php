...
$_GET['BEG_DT']		= '2020--01-02';
$_GET['END_DT']		= date('Y-m-d', strtotime("-1 day", strtotime(date("Y-m-d"))));
...
$query_amt		= "SELECT DATE(HED.ORD_TS) AS ORD_DTE,
			...
			COALESCE(SUM(CASE WHEN (CSH.TAX_APL_ID IN ('I', 'A') AND (CSH.BUY_CO_NBR NOT IN (SELECT CO_NBR FROM NST.PARAM_COMPANY) OR CSH.BUY_CO_NBR IS NULL)) THEN CSH.TND_AMT ELSE 0 END),0) AS OMS_PT,
			
			COALESCE(
				SUM(CASE WHEN ((CSH.TAX_APL_ID NOT IN ('I', 'A') AND CSH.TAX_F = 1 AND (CSH.BUY_CO_NBR NOT IN (SELECT CO_NBR FROM NST.PARAM_COMPANY)))
				OR (CSH.TAX_APL_ID NOT IN ('I', 'A') AND CSH.TAX_F = 2) 
				OR (CSH.TAX_F = 0 AND CSH.TAX_RPT = 1 AND (CSH.BUY_CO_NBR NOT IN (SELECT CO_NBR FROM NST.PARAM_COMPANY) OR CSH.BUY_CO_NBR IS NULL))) 
			THEN CSH.TND_AMT ELSE 0 END),0) AS OMS_CV,
			
			COALESCE(SUM(CASE WHEN (CSH.TAX_APL_ID NOT IN ('I', 'A') AND CSH.TAX_F = 0 AND CSH.TAX_RPT = 0 AND (CSH.BUY_CO_NBR NOT IN (SELECT CO_NBR FROM NST.PARAM_COMPANY))) THEN CSH.TND_AMT ELSE 0 END),0) AS OMS_PR
	FROM 
				( SELECT 
							CSH.TRSC_NBR,
...
$_GET['IVC_TYP'] 	= 'RC';
	...
	try {
		...
		ob_start();
		include __DIR__ . DIRECTORY_SEPARATOR . "ajax/procurement-payment-report.php";

		$results_ivc = json_decode(ob_get_clean());
	} catch (\Exception $ex) {
		ob_end_clean();
	}
		
	
	if($Actg == 1) { 	$InvoiceAmountPT	= $results_ivc->total->TOT_AMT; } 
	if($Actg == 2) { 	$InvoiceAmountCV	= $results_ivc->total->TOT_AMT; }
	if($Actg == 3) { 	$InvoiceAmountPR	= $results_ivc->total->TOT_AMT; }
	

}
*/

$query_ivc_amt = "SELECT 
					SUM(CSH.TND_AMT) AS TND_AMT,
					...
					FROM 
							( SELECT 
								...
								LEFT JOIN RTL.RTL_STK_HEAD HED 
									ON CSH.RTL_BRC = HED.ORD_NBR
								WHERE DATE(CSH.CRT_TS) >= '".$beginDeposit."'
									AND DATE(CSH.CRT_TS) <= (CURRENT_DATE - INTERVAL 1 DAY)
							...
							LEFT JOIN (
								...
								WHERE REG.CSH_FLO_TYP = 'PA'
									AND REG.ACT_F = 0
									AND DATE(REG.CRT_TS) >= '".$beginDeposit."'
									AND DATE(REG.CRT_TS) <= (CURRENT_DATE - INTERVAL 1 DAY)
								GROUP BY REG.TRSC_NBR
							) PYMT
...
$result_ivc_amt	= mysql_query($query_ivc_amt);
$row_ivc_amt	= mysql_fetch_array($result_ivc_amt);


$InvoiceAmountPT	= $row_ivc_amt['IVC_AMT_PT'];
$InvoiceAmountCV	= $row_ivc_amt['IVC_AMT_CV'];
$InvoiceAmountPR	= $row_ivc_amt['IVC_AMT_PR'];
...
$InvoiceAmountPTtoday	= $row_ivc['IVC_AMT_PT'];
$InvoiceAmountCVtoday	= $row_ivc['IVC_AMT_CV'];
$InvoiceAmountPRtoday	= $row_ivc['IVC_AMT_PR'];
...
$satuan_csh 	= 1000000;
	...
	if ((($RevenuePT + $OmsetTodayPT) - $depPT - $InvoiceAmountPT - $InvoiceAmountPTtoday) >= $MinCashPT)	{ $DepositPT	= (($RevenuePT + $OmsetTodayPT) - $depPT - $InvoiceAmountPT - $InvoiceAmountPTtoday) - ($MinCashPT); }	
		else { $DepositPT = 0; }

	if ((($RevenueCV + $OmsetTodayCV + $RevenueRetail + $SalesAmount) - $depCV - $ExpenseAmount - $InvoiceAmountCV - $InvoiceAmountCVtoday) >= $MinCashCV)	{ $DepositCV	= (($RevenueCV + $OmsetTodayCV + $RevenueRetail + $SalesAmount) - $depCV - $ExpenseAmount - $InvoiceAmountCV - $InvoiceAmountCVtoday) - ($MinCashCV); }	
		else { $DepositCV = 0; }
	
	if ((($RevenuePR + $OmsetTodayPR) - $depPR - $InvoiceAmountPR - $InvoiceAmountPRtoday) >= $MinCashPR)	{ $DepositPR	= (($RevenuePR + $OmsetTodayPR) - $depPR - $InvoiceAmountPR - $InvoiceAmountPRtoday) - ($MinCashPR); }	
		else { $DepositPR = 0; }
...
$data['IVC_AMT_PT_TODAY']	= $InvoiceAmountPTtoday;
$data['IVC_AMT_CV_TODAY']	= $InvoiceAmountCVtoday;
$data['IVC_AMT_PR_TODAY']	= $InvoiceAmountPRtoday;
