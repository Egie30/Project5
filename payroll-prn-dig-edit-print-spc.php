<?php
	...
	$query="SELECT PAY.PRSN_NBR
				...
				,PAY.INS_AMT_PRSN
				,PAY.SS_CRD_AMT_PRSN
				,COALESCE(PAY_OUT_CMSN, 0) AS PAY_OUT_CMSN
				,BON_ERNG,ERNG_BNS_AMT
	...
	$header.=followspace(ucfirst($company['ADDRESS']).", ".ucfirst($company['CITY_NM'])." ".ucfirst($company['ZIP']),70).leadspace("Tanggal Gajian : ".$_GET['PYMT_DTE'],65).chr(13).chr(10);
	$header.="Telp ".$ComPhone.", E-Mail: ".$company['EMAIL'].chr(13).chr(10);

	$header.=chr(13).chr(10);
	$customer=trim($row['NAME_PPL']." ".$row['NAME_COM']);
	
	$header.=followspace("Nama Karyawan: Stanley Onggowijaya",70);
    ...
    $string .= followspace("      Gaji pokok             : Rp." . leadSpace(9000000 / $row['PYMT_DAYS'], 7) . " x " . leadSpaceDec($row['BASE_CNT'], 2) . "  hari =   Rp. " . leadspace("9.0000.000", 10), 76) . leadspace("", 10) . followspace("Gaji Pokok Per Bulan", 21) . ":" . " Rp. " . leadspace($people['PAY_BASE'], 11) . "           " . chr(13) . chr(10);
    $string .= followspace("      Bonus bulan ini        : " . leadSpace(0, 2) . " %                    =   Rp. " . leadSpace(0, 10), 76) . leadspace("", 10) . followspace("Gaji Tambahan ", 21) . ":" . " Rp. " . leadspace(0, 11) . "            " . chr(13) . chr(10);
    $string .= followspace("      Komisi                 :                         =   Rp. " . leadspace(0, 10), 76) . leadSpace("", 10) . followspace("Gaji Lembur", 21) . ":" . " Rp. " . leadspace(0, 11) . "            " . chr(13) . chr(10);
    $string .= followspace("      Gaji lembur            : Rp. " . leadSpace(0, 7) . " x " . leadspace(0, 2) . "   jam  =   Rp. " . leadspace(0, 10), 76) . leadspace("", 10) .followspace("Gaji Lain", 21) . ":" . " Rp. " . leadspace(0, 11) . "            " . chr(13) . chr(10);
    $string .= followspace("      Gaji tambahan          : Rp. " . leadSpace(0, 7) . " x " . leadSpaceDec(0, 2) . " hari =   Rp. " . leadspace(0, 10), 76) . leadspace("", 10) . followspace("Gaji Kontribusi", 21) . ":" . " Rp. " . leadspace(0, 11) . "            " . chr(13) . chr(10);
	...
	$string .= followspace("      Transportasi           : Rp. " . leadSpace($Transport, 7) . " x  " . followspace(0, 5) . leadspace('km',3)." =   Rp. " . leadspace(0, 10), 76) . leadspace("", 10) . followspace("Potongan Bon/Bulan", 21) . ":" . " Rp. " . leadspace(0, 11) . "            " .  chr(13) . chr(10);
	if ($row['STY_CNT'] == 0){$menginap = 0;}else{$menginap = $row['STY_TOT_AMT'] / $row['STY_CNT'];}
	$string .= followspace("      Menginap               : Rp. " . leadSpace($menginap, 7) . " x " . leadspace(0, 2) . "   hari =   Rp. " . leadspace(0, 10), 76) . leadspace("", 10) . followspace("Bonus", 21) . ":" . " Rp. " . leadspace(0, 11) . "            " .  chr(13) . chr(10);
	if ($row['PAY_MISC_AMT'] == 0){$paymiscamt = 0;}else{$paymiscamt = $row['PAY_MISC_AMT'];}
	$string .= followspace("      Jasa Pemasangan        :     " .leadspace($row['xx'], 12) . "        =   Rp. " . leadspace(0, 10), 76) . leadspace("", 10) . chr(13) . chr(10);
	...
	$string .= followspace("                                              Jumlah   =   Rp. " . leadSpace('9.0000.000', 10), 76) . leadspace("", 10) . followspace("Sisa Bon", 21) . ":" . " Rp. " . leadspace($people['AV_CRD'], 11).chr(13) . chr(10);
	$string .= followspace("                                       Gaji Kontribusi = - Rp. " . leadSpace(0, 10), 76) . leadspace("", 10) . chr(13) . chr(10);
	$string .= followspace("                                              Bon      = - Rp. " . leadSpace(0, 10), 76) . leadspace("", 10) . chr(13) . chr(10);
    $string .= followspace("                                              Bonus    =   Rp. " . leadSpace(0, 10), 76) . leadspace("", 10) . chr(13) . chr(10);
	
	if ($row['BON_ERNG']>0){
		$string .= followspace("                                    Earning Bonus  =   Rp. " . leadSpace(0, 10), 76) . leadspace("", 10) . chr(13) . chr(10);
	}
	
	if ($row['PAY_OUT_CMSN']>0){
		$string .= followspace("                               Outsource Commission    =   Rp. " . leadSpace(0, 10), 76) . leadspace("", 10) . chr(13) . chr(10);
	}

    //Marketing performance reward 
    if ($row['PAY_RWD_AMT']>0){
    	$string .= followspace("                       Marketing Performance Reward    =   Rp. " . leadSpace(0, 10), 76) . leadspace("", 10) . chr(13) . chr(10);
    }
	...
	$string .= followspace("                                        BPJS Kesehatan = - Rp. " . leadSpace($row['INS_AMT_PRSN'], 10), 76) . leadspace("", 10) . chr(13) . chr(10);
	$string .= followspace("                                  BPJS Ketenagakerjaan = - Rp. " . leadSpace($row['SS_CRD_AMT_PRSN'], 10), 76) . leadspace("", 10) . chr(13) . chr(10);
	
	if($row['PNLTY']>0){
		$pinalty = $row['PNLTY'];
	} else {
		$pinalty = $row['PEER_RWD'];
	}
	
	$string .= followspace("                                  Peer to peer penalty = - Rp. " . leadSpace($pinalty, 10), 76) . " " . chr(13) . chr(10);
if ($row['PAY_HLD_F']=='1'){
