<?php
	...
	$query="SELECT INV.INV_NBR,CONCAT(INV.NAME,' ',COLR_DESC,' ',THIC,' ',SIZE,' ',WEIGHT) AS NAME_DESC,INV_TYP_DESC,
			...
			GROUP BY INV.INV_NBR,INV_TYP_DESC
			ORDER BY 3,2";
	//echo $query;
	...
	while($row=mysql_fetch_array($result))
	{
		...
		if($title!=strtoupper(substr($row['NAME_DESC'],0,1)))
		{
			...
			echo "<th class='listable' colspan='7'colspan-width=20px>Masuk</th>";
			echo "<th class='listable' colspan='7'colspan-width=20px>Keluar</th>";
		...
		echo "<td align='center'>".number_format($row['BEG_VAL'],0,",",".")."</td>";
