...
<form enctype="multipart/form-data" action="" method="post">
    ...
    <div id="upper">
        <div class="left-col">
            <div class="tanggal">
            <label for="BEG_TS">Tanggal Mulai</label>
            <input id="BEG_TS" name="BEG_TS" 
            value="<?php echo $BegDt == "" ? "" : parseDate($BegDt) ?>" required/>
		<script>
			new CalendarEightysix('BEG_TS', { 'offsetY': -5, 'offsetX': 2, 'format': '%Y-%m-%d', 'prefill': false, 'slideTransition': Fx.Transitions.Back.easeOut, 'draggable': true });
		</script>

        <label for="END_TS">Tanggal Selesai</label>
		<input id="END_TS" name="END_TS" 
        value="<?php echo $EndDt; ?>"  />
		<script>
			new CalendarEightysix('END_TS', { 'offsetY': -5, 'offsetX': 2, 'format': '%Y-%m-%d', 'prefill': false, 'slideTransition': Fx.Transitions.Back.easeOut, 'draggable': true });
		</script>
            </div>
            <div class="waktu">
            <label for="BEG_TME">Waktu</label>
                <div class="select-time-wrapper">
                    <input type="hidden" name="BEG_TME" id="BEG_TME">
                    <select class="hour" name="BEG_HR" style="overflow-y: auto;">
                        <?php
                        for ($x = 0; $x <= 23; $x++): ?>
                            <option value="<?php echo $x ?>" <?php echo (parseHour($BegDt) == $x)
                                ? "selected" : "" ?>>
                    ...
                    <select class="minutes" name="BEG_MIN">
                        <?php
                        for ($x = 0; $x <= 60; $x += 5): ?>
                            <option value="<?php echo $x ?>" <?php echo (parseMinute($row['END_TS']) == $x)
                                ? "selected" : "" ?>>
                                <?php echo str_pad($x, 2, "0", 0) ?>
                            </option>
                        <?php
                ...
                <label for="ORD_END_TME">Waktu</label>
                <div class="select-time-wrapper">
                    <input type="hidden" name="ORD_END_TME" id="ORD_END_TME">
                    <select class="hour" name="ORD_END_HR" style="overflow-y: auto;">
                        <?php
                        for ($x = 0; $x <= 23; $x++): ?>
                            <option value="<?php echo $x ?>" <?php echo (parseHour($row['END_TS']) == $x)
                                ? "selected" : "" ?>>
                                <?php echo str_pad($x, 2, "0", 0) ?>
                            </option>
                        <?php
                    ...
                    <select class="minutes" name="ORD_END_MIN">
                        <?php
                        for ($x = 0; $x <= 60; $x += 5): ?>
                            <option value="<?php echo $x ?>" <?php echo (parseMinute($row['END_TS']) == $x)
                                ? "selected" : "" ?>>
        ...
        <tr>
            <td><label for="ORD_TYP">Jenis Layanan</label></td>
            <td>
                <select name="CAT_ID" id="CAT_ID" class="chosen-select">
                    <?php
                    genCombo("SELECT * FROM CMP.RTL_ORD_TYP_CAT", 0, 1, $row['CAT_ID'], "Kategori Layanan")
                    ?>
                </select>
                <br><br>
                <select name="ORD_TYP" id="ORD_TYP" class="chosen-select">
                    ...
                    //                        $query2 = 'SELECT * FROM CMP.RTL_ORD_TYP WHERE CAT_ID=' . $cat;
                    //                        $result2 = mysql_query($query2);
                    //                        while ($typ = mysql_fetch_array($result2)) {
                    //                            $selected = "";
                    //                            if ($row['ORD_TYP'] == $typ['RTL_ORD_TYP']) {
                    //                                $selected = "selected";
                    //                            }
                    //                            echo "<option value='" . $typ['RTL_ORD_TYP'] . "'" . $selected . ">" . $typ['RTL_ORD_DESC']
                    //                                . "</option>";
