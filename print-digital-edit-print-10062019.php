<?php
	...
	$query="SELECT ORD_NBR,DATE_FORMAT(ORD_TS,'%d-%m-%Y') AS ORD_DT,ORD_TS,DATE_FORMAT(CMP_TS,'%d-%m-%Y') AS CMP_DT,STT.ORD_STT_ID,STT.ORD_STT_DESC,BUY_PRSN_NBR,PPL.NAME AS NAME_PPL,COM.NAME AS NAME_COM,BUY_CO_NBR,REF_NBR,ORD_TTL,DUE_TS,PRN_CO_NBR,FEE_MISC,TAX_APL_ID,TAX_AMT,TOT_AMT,PYMT_DOWN,PYMT_REM,TOT_REM,DL_CNT,PU_CNT,NS_CNT,DATE_FORMAT(CMP_TS,'%d-%m-%Y') AS CMP_DT,PU_TS,SPC_NTE,HED.CRT_TS,HED.CRT_NBR,HED.UPD_TS,HED.UPD_NBR,IVC_PRN_CNT,NBR
			FROM ". $headtable ." HED
	...
	if($PrnTyp=="Invoice"){		
		$Suffix="I";
		if(($row['TAX_APL_ID']=="A")||($row['TAX_APL_ID']=="I")){
			$header="Toko Champion    ".pSpace(42)."NOTA PRINTING".pSpace(45)."Nota No. ".leadZero($OrdNbr,6)."-".leadZero($row['IVC_PRN_CNT'],2).chr(13).chr(10);
		}else{
	...
	if(($row['TAX_APL_ID']=="A")||($row['TAX_APL_ID']=="I")){
		$header.="Jl. Pangeran Mangkubumi No. 38, Yogyakarta 55232".pSpace(62)."Tanggal Order: ".$row['ORD_DT'].chr(13).chr(10);
		$header.="Telp (0274) 512078, Fax (0274) 586866        ".pSpace(63)."Tanggal Selesai: ".$row['CMP_DT'].chr(13).chr(10);
	}else{
		$header.=followSpace($CmpDef['ADDRESS'].", ".$CmpDef['CITY_NM']." ".$CmpDef['ZIP'],110)."Tanggal Order: ".$row['ORD_DT'].chr(13).chr(10);
		$header.=followSpace("Telp. ".$CmpDef['PHONE'].", E-Mail: ".$CmpDef['EMAIL'],108)."Tanggal Selesai: ".$row['CMP_DT'].chr(13).chr(10);
	}
	...
	$rowCount=0;
	$pageCount=0;
	$query="SELECT ORD_DET_NBR,DET.ORD_NBR,DET_TTL,DET.PRN_DIG_TYP,PRN_DIG_DESC,DET.PRN_DIG_PRC,ORD_Q,FIL_LOC,PRN_LEN,PRN_WID,FEE_MISC,FAIL_CNT,DISC_PCT,DISC_AMT,VAL_ADD_AMT,TOT_SUB
	...
	while($rowd=mysql_fetch_array($result))
	{
		if($rowd['PRN_DIG_TYP']=='PROD'){
            $query="SELECT ORD_DET_NBR,DET.ORD_NBR,
CLD.ORD_DET_NBR_PAR,DET_TTL,DET.PRN_DIG_TYP,PRN_DIG_DESC,DET.ORD_Q,FIL_LOC,PRN_LEN,PRN_WID,
(COALESCE(CLD.TOT_SUB,0)+COALESCE(CLD.DISC_AMT,0)-COALESCE(CLD.FEE_MISC,0))/COALESCE(DET.ORD_Q,1) AS PRN_DIG_PRC,COALESCE(CLD.FEE_MISC,0)/COALESCE(DET.ORD_Q,1) AS FEE_MISC,COALESCE(CLD.DISC_AMT,0)/COALESCE(DET.ORD_Q,1) AS DISC_AMT,CLD.VAL_ADD_AMT,CLD.TOT_SUB + (COALESCE(DET.FEE_MISC,0) * COALESCE(DET.ORD_Q,1)) AS TOT_SUB,
		COALESCE(DET.FEE_MISC,0) AS DET_FEE_MISC
FROM ". $detailtable ." DET INNER JOIN
(SELECT ORD_DET_NBR_PAR,
SUM(ORD_Q) AS ORD_Q,SUM(FEE_MISC*ORD_Q*COALESCE(PRN_LEN,1)*COALESCE(PRN_WID,1)) AS FEE_MISC,SUM(FAIL_CNT) AS FAIL_CNT,SUM(DISC_AMT*ORD_Q*COALESCE(PRN_LEN,1)*COALESCE(PRN_WID,1)) AS DISC_AMT,SUM(VAL_ADD_AMT) AS VAL_ADD_AMT,SUM(TOT_SUB) AS TOT_SUB
FROM ". $detailtable ." DET
WHERE ORD_DET_NBR_PAR=".$rowd['ORD_DET_NBR']." AND DET.DEL_NBR=0 GROUP BY 1 ORDER BY 1) CLD ON DET.ORD_DET_NBR=CLD.ORD_DET_NBR_PAR
    LEFT OUTER JOIN CMP.PRN_DIG_TYP TYP ON DET.PRN_DIG_TYP=TYP.PRN_DIG_TYP WHERE ORD_DET_NBR=".$rowd['ORD_DET_NBR']." AND DET.DEL_NBR=0 AND DET.PRN_DIG_TYP='PROD' GROUP BY 1,2 ORDER BY 1";
        }else{
            $query="SELECT ORD_DET_NBR,DET.ORD_NBR,ORD_DET_NBR_PAR,DET_TTL,PRN_DIG_DESC,DET.PRN_DIG_PRC,ORD_Q,FIL_LOC,PRN_LEN,PRN_WID,FEE_MISC,FAIL_CNT,DISC_PCT,DISC_AMT,VAL_ADD_AMT,TOT_SUB
			FROM ". $detailtable ." DET LEFT OUTER JOIN
			     CMP.PRN_DIG_TYP TYP ON DET.PRN_DIG_TYP=TYP.PRN_DIG_TYP
			WHERE ORD_DET_NBR=".$rowd['ORD_DET_NBR']." OR ORD_DET_NBR_PAR=".$rowd['ORD_DET_NBR']." AND DET.DEL_NBR=0 ORDER BY 1";
        }
        //echo $query;
