<?php
	...
	//echo $query;
	$query="SELECT ORD_NBR,ORD_DTE,REF_NBR,REQ_NBR,ORD_TYP,SEL_CO_NBR,BUY_CO_NBR,ORD_TTL,PRN_DTE,PRN_CO_NBR,FEE_FLM,FEE_MISC,CMP_DTE,PYMT_DOWN,PYMT_REM,TOT_AMT,SPC_NTE,PU_DTE,TOT_REM,TAX_APL_ID,TAX_AMT FROM CMP.CAL_ORD_HEAD WHERE ORD_NBR=".$OrdNbr;
	//echo $query;
	$result=mysql_query($query);
	$row=mysql_fetch_array($result);
	$ordDte = $row['ORD_DTE'];
	...
	if ($PrnTyp=="SL" || $PrnTyp=="PO"){
		...
		*/
		if($taxType=="I"){
			$string.=pSpace(98)."Jumlah PPN";
			$string.=leadSpace($taxAmount,15).chr(13).chr(10);
		}else{
			$string.=pSpace(105)."PPN";
			$string.=leadSpace($taxAmount,15).chr(13).chr(10);
		}
		$string.="      Pembuat     ".pSpace(5)."     Penerima     ".pSpace(5)."     Mengetahui      ".pSpace(47)."TOTAL".leadSpace($totalAmount,15).chr(13).chr(10).chr(13).chr(10).chr(13).chr(10);
		$string.="(________________)".pSpace(5)."(_________________)".pSpace(5)."(_________________)";
	}
	$string.=chr(13).chr(10);
