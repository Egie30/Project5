...
$query  = "DELETE FROM CDW.PAY_OUT_CMSN WHERE PAY_CONFIG_NBR = ".$PayConfigNbr;
//echo $query."<br/><br/>";
$result = mysql_query($query);

$query = "INSERT INTO CDW.PAY_OUT_CMSN
	SELECT 
	STK.ORD_DET_NBR,
	STK.ORD_NBR,
	STK.INV_NBR,
...
FROM CMP.PRN_DIG_ORD_HEAD HED 
	...
	INNER JOIN (
		SELECT
			...
			COALESCE(PAR.TOT_SUB, 0) AS PAR_TOT_SUB,
			SUM(COALESCE(DET.TOT_SUB, 0) + COALESCE(PAR.TOT_SUB, 0)) AS SELL_TOT_SUB
		...
		LEFT OUTER JOIN(
			...
			WHERE DEL_NBR = 0 AND ORD_DET_NBR_PAR != ''
			GROUP BY ORD_DET_NBR_PAR
		)PAR ON DET.ORD_DET_NBR = PAR.ORD_DET_NBR_PAR
		WHERE DEL_NBR = 0 AND DET.ORD_DET_NBR_PAR IS NULL
...
WHERE 
	HED.DEL_NBR = 0 
	AND DATE(STK.ORD_DTE) >= '$PayBegDte'
	AND DATE(STK.ORD_DTE) <= '$PayEndDte'
GROUP BY DET.ORD_DET_NBR";
